...
class _UpdateArticlePageMobileState extends State<UpdateArticlePageMobile> {
  ...
  Future<void> _continue() async {
    if (_titleController.text.isEmpty || _quillController.document.isEmpty()) {
      ...
      if (_coverImage != null) {
        await context
            .read<ArticleModel>()
            .setCoverImage(_coverImage!.readAsBytes(), widget.articleId);
      }
      // ignore: use_build_context_synchronously
      await context.read<ArticleModel>().updateArticleWithId(widget.articleId, {
        'title': _titleController.text,
        'content': jsonEncode(_quillController.document.toDelta().toJson()),
      });
      Modular.to.pushNamed(
        '$updateArticleRoute/cont?articleId=${widget.articleId}',
      );
    }
  }
  ...
  Widget build(BuildContext context) {
    return MobileScaffold(
      ...
      body: FutureBuilder(
        future: context.read<ArticleModel>().getArticleFromId(widget.articleId),
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            _quillController.document = Document.fromJson(
                jsonDecode(snapshot.data!.data()!['content']));
            _titleController.text = snapshot.data!.data()!['title'];
  ...
  Widget _buildPage(BuildContext context, Map<String, dynamic> articleData) {
    return Form(
      child: Column(
        children: [
          Builder(
...
class UpdateArticlePageMobileCont extends StatefulWidget {
  final String articleId;
  const UpdateArticlePageMobileCont(this.articleId, {super.key});

  @override
  State<UpdateArticlePageMobileCont> createState() =>
      _UpdateArticlePageMobileContState();
}

class _UpdateArticlePageMobileContState
    extends State<UpdateArticlePageMobileCont> {
  ...
  Future<void> _updateArticle() async {
    if (_selectedCity == null || _tagsController.getTags!.isEmpty) {
      ...
      await ArticleModel().updateArticleWithId(widget.articleId, {
  ...
  Widget build(BuildContext context) {
    return MobileScaffold(
        actions: [
          IconButton(
              onPressed: () => _updateArticle(), icon: const Icon(Icons.save))
        ],
        body: FutureBuilder(
          future:
              context.read<ArticleModel>().getArticleFromId(widget.articleId),
          builder: (context, snapshot) {
            if (snapshot.connectionState == ConnectionState.waiting) {
              return const Center(
                child: CircularProgressIndicator(),
              ...
              _datePublished = snapshot.data!.data()!['datePublished'].toDate();
              _selectedCity = snapshot.data!.data()!['city'];
              _initialTags = List<String>.from(snapshot.data!.data()!['tags']);
              return _buildPage(context, snapshot.data!.data()!);
            }
          },
        ));
